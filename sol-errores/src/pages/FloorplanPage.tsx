...
export default function FloorplanPage() {
  ...
  // Construir la URL correcta de la imagen del plano:
  // - Si VITE_API_URL es relativo (/api), la URL del backend ya es una ruta relativa y el
  //   navegador la resolverá correctamente (ej: /api/floorplan/image-file/xxx).
  // - Si VITE_API_URL es absoluto (http://...), necesitamos anteponer la base, quitando
  //   el prefijo /api del path almacenado para no duplicarlo.
  const buildImageUrl = (raw: string | null): string | null => {
    if (!raw) return null;
    const apiUrl = (import.meta.env.VITE_API_URL as string) || '/api';
    if (apiUrl.startsWith('http')) {
      // URL absoluta: sustituir el prefijo /api por la base completa
      const base = apiUrl.endsWith('/api') ? apiUrl.slice(0, -4) : apiUrl.replace(/\/api$/, '');
      return base + raw; // raw ya empieza por /api/...
    }
    // URL relativa: la devolvemos tal cual, el navegador la resuelve desde el mismo origen
    return raw;
  };
  const currentImage = buildImageUrl(rawImage);
