...
import { X, Save, User, FileText, ExternalLink, History, Info as InfoIcon, ClipboardList, Package } from 'lucide-react';
import { Asset, AssetCategory, AssetStatus, ClientUser, AssetUserLink } from '../types';
import DocumentsPanel from './DocumentsPanel';
import AuditLogPanel from './AuditLogPanel';
import AssetSystemInfoPanel from './AssetSystemInfoPanel';
import AssetSoftwarePanel from './AssetSoftwarePanel';
...
type AssetTab = 'basic' | 'info' | 'software';
...
export default function AssetForm({ asset, onSave, onClose, isEdit }: AssetFormProps) {
  ...
  // La pestana Info solo existe para activos ya creados (necesita un asset.id
  // para persistir notas en asset_system_info). En "Nuevo activo" se oculta.
  const [tab, setTab] = useState<AssetTab>('basic');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl">
        ...
        {/* Tabs — solo si estamos editando un activo existente */}
        {isEdit && asset && (
          <div className="flex items-center gap-1 px-6 pt-3 border-b border-gray-800 bg-gray-900/40">
            <button type="button" onClick={() => setTab('basic')}
              className={`flex items-center gap-1.5 px-4 py-2 text-sm font-medium border-b-2 transition-colors ${tab === 'basic' ? 'border-blue-500 text-white' : 'border-transparent text-gray-400 hover:text-gray-200'}`}>
              <ClipboardList className="w-4 h-4" /> Datos básicos
            </button>
            <button type="button" onClick={() => setTab('info')}
              className={`flex items-center gap-1.5 px-4 py-2 text-sm font-medium border-b-2 transition-colors ${tab === 'info' ? 'border-blue-500 text-white' : 'border-transparent text-gray-400 hover:text-gray-200'}`}>
              <InfoIcon className="w-4 h-4" /> Info del sistema
            </button>
            <button type="button" onClick={() => setTab('software')}
              className={`flex items-center gap-1.5 px-4 py-2 text-sm font-medium border-b-2 transition-colors ${tab === 'software' ? 'border-blue-500 text-white' : 'border-transparent text-gray-400 hover:text-gray-200'}`}>
              <Package className="w-4 h-4" /> Software instalado
            </button>
          </div>
        )}

        {/* Panel Info del sistema (resumen tipo Lansweeper + notas manuales) */}
        {isEdit && asset && tab === 'info' && (
          <div className="flex-1 overflow-y-auto px-6 py-5">
            <AssetSystemInfoPanel assetId={asset.id} />
          </div>
        )}

        {/* Panel Software instalado (lista detectada via WMI/registro Uninstall) */}
        {isEdit && asset && tab === 'software' && (
          <div className="flex-1 overflow-y-auto px-6 py-5">
            <AssetSoftwarePanel assetId={asset.id} />
          </div>
        )}

        {/* Formulario */}
        <form onSubmit={handleSubmit} className={`flex-1 overflow-y-auto px-6 py-5 ${isEdit && asset && tab !== 'basic' ? 'hidden' : ''}`}>
