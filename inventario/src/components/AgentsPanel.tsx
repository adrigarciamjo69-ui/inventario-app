...
export default function AgentsPanel() {
  ...
  // Comandos one-liner: el backend genera un script de instalacion personalizado
  // con URL y enroll_key embebidas. El admin solo pega 1 linea en el equipo destino.
  const winInstall  = `powershell -ExecutionPolicy Bypass -Command "iwr '${serverUrl}/api/agent/install/windows?key=${enrollKey}' -UseBasicParsing | iex"`;

  const linuxInstall = `curl -fsSL '${serverUrl}/api/agent/install/linux?key=${enrollKey}' | sudo bash`;

  const macInstall   = `curl -fsSL '${serverUrl}/api/agent/install/darwin?key=${enrollKey}' | sudo bash`;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Instalacion con un unico comando */}
      <div className="bg-gradient-to-br from-blue-500/10 to-purple-500/5 border border-blue-500/30 rounded-xl p-5 space-y-4">
        <div>
          <h3 className="text-white font-medium flex items-center gap-2">
            <TerminalIcon className="w-4 h-4" /> Instalacion con un solo comando
            <span className="text-[10px] px-2 py-0.5 bg-blue-500/20 text-blue-300 border border-blue-500/30 rounded-full uppercase tracking-wider">Recomendado</span>
          </h3>
          <p className="text-gray-400 text-sm mt-1">
            Copia el comando del SO destino y pegalo en una terminal con privilegios de administrador.
            Descarga el agente, lo enrola y registra el servicio automaticamente.
          </p>
        </div>
        {([['Windows', winInstall, 'PowerShell como Administrador'], ['Linux', linuxInstall, 'terminal con sudo'], ['macOS', macInstall, 'terminal con sudo']] as const).map(([os, cmd, ctx]) => (
          <div key={os}>
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm text-gray-200 font-medium flex items-center gap-1.5">
                <TerminalIcon className="w-3.5 h-3.5" />
                {os}
                <span className="text-[11px] text-gray-500 font-normal">({ctx})</span>
              </span>
              <button onClick={() => copy(cmd, os)} className="text-xs px-2.5 py-1 bg-blue-500/15 hover:bg-blue-500/25 text-blue-200 border border-blue-500/30 rounded flex items-center gap-1">
                <Copy className="w-3 h-3" /> Copiar comando
              </button>
            </div>
            <pre className="bg-black/50 border border-gray-800 rounded p-3 text-xs text-blue-100 overflow-x-auto whitespace-pre-wrap break-all">{cmd}</pre>
          </div>
        ))}
        <details className="text-xs text-gray-400">
          <summary className="cursor-pointer text-gray-300 hover:text-gray-100">¿Que hace exactamente este comando?</summary>
          <ol className="list-decimal pl-5 mt-2 space-y-0.5">
            <li>Descarga un script de instalacion desde tu servidor con la URL y la enroll_key ya embebidas.</li>
            <li>Descarga el binario del agente para el SO correspondiente.</li>
            <li>Ejecuta <code className="text-gray-200">enroll</code> contra el backend para registrar el equipo y obtener su token permanente.</li>
            <li>Registra el agente como servicio del sistema (servicio de Windows / systemd / LaunchDaemon).</li>
            <li>Arranca el servicio. A partir de aqui, el equipo aparece en la lista de abajo y se sincroniza solo.</li>
          </ol>
          <p className="mt-2 text-gray-500">Si rotas la enroll_key arriba, vuelve a copiar el comando — el anterior dejara de funcionar para nuevos enrolamientos (los equipos ya enrolados siguen activos).</p>
        </details>
      </div>
