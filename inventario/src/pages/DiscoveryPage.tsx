...
import {
  ...
  CalendarClock, Mail, Play, Filter,
} from 'lucide-react';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-lg shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1">Expresión cron *</label>
                <input className={inputCls + ' font-mono'} value={schedForm.cron_expr}
                  onChange={(e) => setSchedForm((f) => ({ ...f, cron_expr: e.target.value }))}
                  placeholder="0 3 * * *" />
                <p className="text-[11px] text-gray-500 mt-1">
                  Formato: <span className="font-mono">min hora día mes diaSemana</span>.
                  Ej: <span className="font-mono">0 3 * * *</span> = todos los días a las 03:00.
                </p>
              </div>
              <div>
                    ...
                    <tbody>
                      {rows.map((r, idx) => (
                        ...
                        <tr key={r.result_id} className={`border-b border-gray-800/50 ${r.imported ? 'opacity-50' : ''}`}>
                          ...
                          <td className="p-2 align-top"><input className={cellCls} value={r.assigned_to} disabled={r.imported} onChange={(e) => setRow(idx, { assigned_to: e.target.value })} placeholder="Nombre persona" /></td>
