  ...
  return new Promise((resolve) => {
    ...
    proc.on('close', (code) => {
      ...
      try {
        ...
        // Lista de software instalado (registro Uninstall via StdRegProv).
        if (Array.isArray(j.software)) {
          data.software = j.software;
        }
        if (Object.keys(data).length) return resolve({ data, error: null });
...
function buildResult(host, enrich, method) {
  const e = enrich || {};
  // system_info: bloque detallado (CPU/RAM/discos/red/AD/AV...) que devuelve
  // wmi_query.py o las funciones SSH/SNMP enriquecidas. Si el enrich no lo
  // trae, dejamos null para que el frontend trate el equipo como "sin info".
  return {
    ip: host.ip,
    mac: host.mac || null,
    ...
    system_info: e.system_info || null,
    software: Array.isArray(e.software) ? e.software : null,
    raw: { host, enrich: e },
  };
}
