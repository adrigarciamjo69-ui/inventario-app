...
// GET /api/client-users/:id/assets — activos asignados a un usuario
router.get('/:id/assets', authenticate, async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT
        a.id, a.serial_number, a.category, a.brand, a.model,
        a.status, a.notes, a.assigned_to,
        aul.link_type, aul.assigned_at,
      ...
      WHERE aul.client_user_id = $1
      ORDER BY aul.assigned_at DESC
    `, [req.params.id]);
    res.json(rows);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Error al obtener activos del usuario' });
  }
});
