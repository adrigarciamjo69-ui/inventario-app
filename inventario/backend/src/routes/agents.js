...
// Elimina bytes nulos (\u0000) recursivamente. PostgreSQL TEXT no los admite
// y algunos equipos los traen en campos de software/hardware -> causa 500.
function stripNulls(v) {
  ...
  if (typeof v === 'object') {
    const out = {};
    for (const k of Object.keys(v)) out[k] = stripNulls(v[k]);
    return out;
  }
  return v;
}
...
router.post('/report', authenticateAgent, async (req, res) => {
  ...
  const r = stripNulls(normalizeReport(req.body || {}));
