...
// GET /api/assets/:id/software — devuelve el inventario de software instalado
// detectado en el ultimo escaneo (refrescado al importar/escanear el host).
router.get('/:id/software', authenticate, async (req, res) => {
  try {
    const exists = await pool.query('SELECT 1 FROM assets WHERE id=$1', [req.params.id]);
    if (!exists.rowCount) return res.status(404).json({ error: 'Activo no encontrado' });
    const { rows } = await pool.query(
      `SELECT asset_id, software, scanned_at, source, updated_at
         FROM asset_software WHERE asset_id=$1`,
      [req.params.id]
    );
    if (!rows.length) {
      return res.json({
    ...
    console.error(err);
    res.status(500).json({ error: 'Error al obtener el software del activo' });
  }
});
