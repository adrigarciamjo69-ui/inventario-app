...
// Devuelve true si el activo esta siendo gestionado por un agente endpoint
// activo. En ese caso los escaneos de red no deben pisar asset_system_info
// ni asset_software (el agente tiene los datos mas frescos).
async function isManagedByAgent(assetId) {
...
async function runJob(jobId, network, credentials) {
  try {
    ...
    for (const r of results) {
      ...
      // Si el resultado se ha asociado a un activo existente y traemos
      // info detallada del sistema (CPU/RAM/discos/red/AD...), refrescamos
      // el resumen en asset_system_info preservando las notas manuales.
      const skipAgentManaged = await isManagedByAgent(matched);
      if (skipAgentManaged) {
        console.log(`[scan] ${matched} gestionado por agente, no se sobreescribe asset_system_info/software`);
      }
      if (matched && r.system_info && !skipAgentManaged) {
        try {
          ...
          console.warn('asset_system_info upsert (existing match) fallo:', e.message);
        }
      }
      // Mismo patron para el inventario de software instalado.
      if (matched && Array.isArray(r.software) && r.software.length && !skipAgentManaged) {
        try {
          ...
          console.warn('asset_software upsert (existing match) fallo:', e.message);
        }
      }
    }
    await pool.query(
...
router.post('/jobs/:id/import', authenticate, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    for (const a of items) {
      ...
      try {
        const purchaseDate = a.purchase_date && String(a.purchase_date).trim() ? String(a.purchase_date).trim() : null;
        const purchaseOrder = a.purchase_order?.trim() || null;
        const result = await client.query(
          `INSERT INTO assets (id, serial_number, category, brand, model, price, purchase_date, purchase_order, assigned_to, department, status, notes)
           ...
           ON CONFLICT (id) DO UPDATE SET
             ...
             purchase_date=EXCLUDED.purchase_date, purchase_order=EXCLUDED.purchase_order,
        ...
        if (a.result_id) {
          ...
          // Vuelca el system_info del resultado al resumen del activo (Info tab).
          // Preserva manual_notes (solo actualiza scanned_data + scanned_at).
          try {
            const sr = await client.query(
              'SELECT system_info, software, enrich_method FROM scan_results WHERE id=$1',
              [a.result_id]
            );
            const sysInfo = sr.rows[0] && sr.rows[0].system_info;
            const sw      = sr.rows[0] && sr.rows[0].software;
            const method  = sr.rows[0] && sr.rows[0].enrich_method;
            // No pisar datos del agente si el activo esta gestionado por uno.
            const mng = await client.query(
              `SELECT 1 FROM agent_devices WHERE asset_id=$1 AND status='active' LIMIT 1`,
              [String(a.id).trim()]
            );
            const skipAgentManaged = mng.rowCount > 0;
            if (sysInfo && !skipAgentManaged) {
              await client.query(
                `INSERT INTO asset_system_info (asset_id, scanned_data, scanned_at, source, updated_at)
                 VALUES ($1, $2, NOW(), $3, NOW())
                 ON CONFLICT (asset_id) DO UPDATE SET
                   ...
                   source       = EXCLUDED.source,
                   updated_at   = NOW()`,
                [String(a.id).trim(), JSON.stringify(sysInfo), method]
              );
            }
            if (Array.isArray(sw) && sw.length && !skipAgentManaged) {
              await client.query(
                `INSERT INTO asset_software (asset_id, software, scanned_at, source, updated_at)
                 VALUES ($1, $2, NOW(), $3, NOW())
                 ON CONFLICT (asset_id) DO UPDATE SET
                   ...
                   source     = EXCLUDED.source,
                   updated_at = NOW()`,
                [String(a.id).trim(), JSON.stringify(sw), method]
              );
            }
          } catch (e) {
            console.warn('asset_system_info/software upsert (import) fallo:', e.message);
          }
        }
      } catch (rowErr) {
...
// ============================================================================
// FASE 3 - Escaneos programados (cron) + badge de nuevos
// ============================================================================
...
function validateCron(expr) {
  ...
  try { return cronLib.validate(expr.trim()); } catch { return false; }
}

function normalizeEmails(input) {
  if (!input) return [];
  const arr = Array.isArray(input) ? input : String(input).split(/[,;\s]+/);
  return arr.map((e) => String(e).trim()).filter((e) => /.+@.+\..+/.test(e));
}

// GET /api/scan/schedules
router.get('/schedules', authenticate, requireAdmin, async (_req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT s.*, n.name AS network_name, n.cidr AS network_cidr
         FROM scan_schedules s
         LEFT JOIN scan_networks n ON n.id = s.network_id
        ORDER BY s.id DESC`
    );
    res.json(rows);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Error al obtener los escaneos programados' });
  }
});

// POST /api/scan/schedules
router.post('/schedules', authenticate, requireAdmin, async (req, res) => {
  const { name, network_id, cron_expr, timezone, enabled, notify_emails } = req.body;
  if (!name || !network_id || !cron_expr) {
    return res.status(400).json({ error: 'name, network_id y cron_expr son obligatorios' });
  }
  if (!validateCron(cron_expr)) {
    return res.status(400).json({ error: 'Expresion cron no valida' });
  }
  try {
    const { rows } = await pool.query(
      `INSERT INTO scan_schedules (name, network_id, cron_expr, timezone, enabled, notify_emails)
       VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
      [
        String(name).trim(),
        parseInt(network_id),
        String(cron_expr).trim(),
        (timezone && String(timezone).trim()) || 'Europe/Madrid',
        enabled !== false,
        JSON.stringify(normalizeEmails(notify_emails)),
      ]
    );
    try { require('../scheduler').reloadAll(); } catch (_) {}
    res.status(201).json(rows[0]);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Error al crear el escaneo programado' });
  }
});

// PUT /api/scan/schedules/:id
router.put('/schedules/:id', authenticate, requireAdmin, async (req, res) => {
  const { name, network_id, cron_expr, timezone, enabled, notify_emails } = req.body;
  if (cron_expr !== undefined && !validateCron(cron_expr)) {
    return res.status(400).json({ error: 'Expresion cron no valida' });
  }
  try {
    const { rows: cur } = await pool.query('SELECT * FROM scan_schedules WHERE id=$1', [req.params.id]);
    ...
    const { rows } = await pool.query(
      ...
      [
        ...
        network_id !== undefined ? parseInt(network_id) : c.network_id,
        cron_expr !== undefined ? String(cron_expr).trim() : c.cron_expr,
        timezone !== undefined ? (String(timezone).trim() || 'Europe/Madrid') : c.timezone,
        enabled !== undefined ? !!enabled : c.enabled,
        notify_emails !== undefined ? JSON.stringify(normalizeEmails(notify_emails)) : JSON.stringify(c.notify_emails || []),
        req.params.id,
      ]
    );
    try { require('../scheduler').reloadAll(); } catch (_) {}
    ...
    console.error(err);
    res.status(500).json({ error: 'Error al actualizar el escaneo programado' });
  }
});

// DELETE /api/scan/schedules/:id
router.delete('/schedules/:id', authenticate, requireAdmin, async (req, res) => {
  try {
    const { rowCount } = await pool.query('DELETE FROM scan_schedules WHERE id=$1', [req.params.id]);
    if (rowCount === 0) return res.status(404).json({ error: 'No encontrado' });
    try { require('../scheduler').unregister(parseInt(req.params.id)); } catch (_) {}
    res.json({ message: 'Escaneo programado eliminado' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Error al eliminar el escaneo programado' });
  }
});

// POST /api/scan/schedules/:id/run-now  -> ejecuta el schedule manualmente
router.post('/schedules/:id/run-now', authenticate, requireAdmin, async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT * FROM scan_schedules WHERE id=$1', [req.params.id]);
    if (rows.length === 0) return res.status(404).json({ error: 'No encontrado' });
    require('../scheduler').runSchedule(rows[0]).catch((e) => console.error('run-now error', e));
    res.json({ message: 'Escaneo lanzado' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Error al lanzar el escaneo' });
  }
});

// GET /api/scan/discoveries/new-count
// Resultados de escaneos (de cualquier job) pendientes y NO inventariados.
router.get('/discoveries/new-count', authenticate, async (_req, res) => {
  try {
    const { rows } = await pool.query(
    ...
    console.error(err);
    res.status(500).json({ error: 'Error al contar nuevos descubrimientos' });
  }
});
