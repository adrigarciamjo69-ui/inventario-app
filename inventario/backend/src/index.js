...
async function initDB() {
  ...
  try {
    ...
    await client.query(`
      ...
      -- Agentes endpoint (Windows/Linux/macOS) que reportan datos sin necesitar
      -- credenciales. Cada equipo se enrola una unica vez con la enroll_key
      -- compartida y recibe un token permanente; despues hace checkin periodico.
      CREATE TABLE IF NOT EXISTS agent_devices (
        id              SERIAL PRIMARY KEY,
        machine_id      VARCHAR(100) UNIQUE NOT NULL,    -- UUID estable generado por el agente
        hostname        VARCHAR(255),
        os              VARCHAR(20),                     -- 'windows' | 'linux' | 'darwin'
        os_version      VARCHAR(100),
        agent_version   VARCHAR(20),
        token_hash      VARCHAR(128) NOT NULL,           -- sha256 hex del token plano
        asset_id        VARCHAR(50) REFERENCES assets(id) ON DELETE SET NULL,
      ...
      CREATE INDEX IF NOT EXISTS idx_agent_devices_asset ON agent_devices(asset_id);
      ...
      -- Cache del ultimo system_info / software reportado por el agente, para:
      --  (1) poder mostrarlo retroactivamente cuando el admin vincula manualmente
      --      el agente a un activo (datos del checkin previo).
      --  (2) que la columna asset_system_info se rellene incluso en el primer
      --      checkin tras enrolar.
      ALTER TABLE agent_devices ADD COLUMN IF NOT EXISTS last_system_info JSONB;
      ALTER TABLE agent_devices ADD COLUMN IF NOT EXISTS last_software    JSONB;
      ALTER TABLE agent_devices ADD COLUMN IF NOT EXISTS last_checkin_at  TIMESTAMPTZ;
    `);
  } finally {
    client.release();
  }
